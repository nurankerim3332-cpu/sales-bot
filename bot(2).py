...
SHEET_NAME   = 'Sales'
DAILY_HOUR   = 20  # 20:00 по Бишкеку
...
async def daily_scheduler(app: Application):
    """Фоновая задача — отправляет статистику каждый день в DAILY_HOUR:00."""
    while True:
        now = datetime.now(TZ)
        ...
        logger.info(f'Следующая отправка через {wait_sec/3600:.1f} ч ({target.strftime("%d.%m %H:%M")})')
