...
def open_btn(in_group=False):
    """
    В личном чате — Mini App кнопка (открывается прямо в Telegram).
    В группе — обычная ссылка (WebApp не работает в группах).
    """
    if in_group:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Защита от None (может случиться при channel posts)
...
async def daily_scheduler(bot):
    while True:
        ...
        try:
            # Для плановой рассылки в группу всегда используем обычную ссылку
...
async def main():
    ...
    # filters.ALL позволяет боту отвечать и в группах, и в личке
    ...
    async with app:
        ...
        await app.updater.start_polling(
            ...
            drop_pending_updates=True   # игнорировать накопившиеся старые сообщения
        )
