...
SHEET_NAME   = 'Sales'
DAILY_HOUR   = 20   # время отправки (20:00 Бишкек UTC+6)
DAILY_MIN    = 0
...
def build_report():
    ...
    for i, (name, d) in enumerate(sorted_emp):
        ...
        if d['salary'] > 0:
            ...
            icon    = '📈' if pct > 0 else '📉'
            lines.append(f"{medal} *{name}*\n   💰 {fmt_money(d['earned'])} сом  {icon} {pct_str}\n")
        else:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(text, parse_mode='Markdown', reply_markup=open_btn())
...
async def send_daily(context: ContextTypes.DEFAULT_TYPE):
    logger.info('Отправляю ежедневную статистику...')
...
def main():
    ...
    # Ежедневная отправка в 20:00 по Бишкеку (UTC+6 = 14:00 UTC)
    app.job_queue.run_daily(
        send_daily,
        time=datetime.now(TZ).replace(hour=DAILY_HOUR, minute=DAILY_MIN, second=0).timetz()
    )
